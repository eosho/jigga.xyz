...
show_local_routing() {
    ...
    nslookup kampfzwerg.jigga.xyz 2>&1 | tee -a "$LOG_FILE" || echo "DNS lookup failed" | tee -a "$LOG_FILE"
    nslookup ethos.jigga.xyz 2>&1 | tee -a "$LOG_FILE" || echo "DNS lookup failed" | tee -a "$LOG_FILE"
